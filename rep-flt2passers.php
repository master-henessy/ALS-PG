...
function Header()
{
	...
	$this->Cell(0,7,'LIST OF PASSERS',0,0,'L');
	$this->Ln(7);
}
...
// Arial bold 15
	...
	$pdf->Ln(5);
	$pdf->Cell(190.00,7,'LIST OF PASSERS',0,0,'C');
	// Line break
	// Arial bold 15
	...
	$pdf->Cell(16);
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(10.00,5,' '.$repsy.' ',0,0,'L');
	$pdf->Cell(35);
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(10.00,5,'Exam Name:',0,0,'L');
	$pdf->Cell(20);
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(10.00,5,$etname.' EXAM',0,0,'L');
	$pdf->Cell(30);
	$pdf->SetFont('Arial','B',12);
	...
	$pdf->Ln(5);
	$sql2 = "SELECT * FROM exams LEFT JOIN students ON exams.student_id = students.Student_id where exams.sy='$repsy' AND flt2stat ='PASS'";
                          $result2 = mysqli_query($conn, $sql2);
                          while ($row2 = mysqli_fetch_array($result2)){
                            $student_id = $row2["student_id"];
                            $flt2 = $row2["flt2"];
                            $flt2_noi = $row2["flt2_noi"];
                            $flt2stat = $row2["flt2stat"];
