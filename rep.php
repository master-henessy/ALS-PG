...
// Arial bold 15
	...
	$pdf->Cell(20.00,5,'Student #','T,L,R',0,'C');
	$pdf->Cell(.01);
	$pdf->Cell(100.00,5,'Name','T,L,R',0,'C');
	$pdf->Cell(.01);
	$pdf->Cell(60.00,5,'Contact #','T,L,R',0,'C');	
	$pdf->Ln(5);
	$sql=mysqli_query($conn,"select * from students where sy='$sy' AND sec_no='$sec_no'")or die(mysqli_error($conn));
    ...
    $pdf->SetFont('Arial','',10);
    $pdf->Cell(20.00,5,$sid,1,0,'C');
	$pdf->Cell(.01);
	$pdf->Cell(100.00,5,$fname." ".$mdname." ".$lname,1,0,'C');
	$pdf->Cell(.01);
	$pdf->Cell(60.00,5,$cnumber,1,0,'C');	
	$pdf->Ln(5);
